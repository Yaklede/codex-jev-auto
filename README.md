...
새 라우팅 기록에는 원문 없이 세션 식별자의 해시, 직전 route key, `explicit_followup`·`possible_correction`·`new_request` 관계 신호, Jev 선택 점수·후보 수·저장소 프로필, 라우팅 정책과 주입 가이드의 소스 해시도 남긴다. 프록시가 재시작해도 같은 장기 세션의 이전 기록을 연결할 수 있다. `auto-runs`에서 이 필드를 조회할 수 있다. 관계 신호는 작업 성공·실패 판정이 아니며, 안정적인 세션 식별자를 클라이언트가 보내지 않으면 연결은 `unlinked`로 남는다. 기존 기록에는 새 필드가 없고, 사용자 수정 지적이 실제 라우팅 오류였는지는 검증·화면·피드백과 대조해야 한다. 이 기록 자체가 자동 재학습을 수행하지는 않는다.
